...
def classify_and_extract():
    ...
    model = Model(model=models.resnet50(pretrained=True), gpu=args.gpu)  # 首先定义了一个对象
    ...
    features = np.empty(shape=(data.num_samples, 2048))  # Return a new array of given shape and type, without initializing entries.
    ...
    for i, d in enumerate(data):
        ...
        sys.stdout.write('\r%d/%d samples completed' % (i + 1, data.num_samples))  # sys.stdout是python中标准输出流
        sys.stdout.flush()  # 就是显示地让缓冲区的内容输出
