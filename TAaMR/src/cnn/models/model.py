...
class Model:
    ...
    def classification(self, list_classes, sample):
        ...
        output = torch.nn.functional.softmax(input=self.model(image[None, ...].to(self.device)), dim=1)  # 其中'None'即加入了一个维度，这里其实就是加入了一个batch_size,而...表示的是保留图像的原维度
    ...
    def feature_extraction(self, sample):
        ...
        if len(sample) != 1:
            ...
            image = sample
            feature = np.squeeze(self.feature_model(image.to(self.device)).data.cpu().numpy())
            return feature
        ...
        if self.feature_model:
            feature = np.squeeze(self.feature_model(image[None, ...].to(self.device)).data.cpu().numpy())  # np.squeeze()删除为1的维度
            return feature

    def set_classify_model(self, save_layers):
        self.classify_model = torch.nn.Sequential(*list(self.model.children())[-save_layers:])
        if self.eval:
            self.feature_model.eval()

    def classify(self, sample):
        feature = sample
        output = torch.nn.functional.softmax(input=self.classify_model(feature.to(self.device)), dim=0)
        return output
