...
class Dataset:
    def __init__(self, args):
        ...
        self.emb_image = np.load(self.f_feature)  # 这里就是图像特征
        self.fsz = self.emb_image.shape[1]  # fsz就是feature size图像特征的数量
        ...
        self.emb_image = self.emb_image / np.max(np.abs(self.emb_image))  # 这其中np.max()会选择整个矩阵中最大的值，相当于进行了归一化处理吧
        ...
        self.item_image_lists = [f"{item}.jpg" for item in range(self.isz)]
