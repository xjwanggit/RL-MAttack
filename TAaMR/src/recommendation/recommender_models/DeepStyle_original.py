...
class DeepStyle:
    ...
    def __init__(self, args, num_users, num_items, num_image_feature):
        ...
        self.index_to_image_path = {i: f"'../../../data/{args.dataset}/{args.experiment_name}/images/{i}.jpg" for i in
                                    range(num_items)}
        # self.item_image_lists = [f"'../../../data/{args.dataset}/{args.experiment_name}/images/{item}.jpg" for item in range(num_items)]
        self.item_category = pd.read_csv(f'../data/{args.dataset}/{args.experiment_name}/classes.csv')['ClassNum'].tolist()
        self.gpu_setting()
    ...
    def gpu_setting(self):
        ...
        if gpus:
            try:
                ...
                # Optionally, specify the specific GPU to use (e.g., GPU:0)
    ...
    def _specific_perdiction(self):
        with tf.name_scope("specific_prediction"):
            ...
            # Get category indices for the items
            item_category_indices = tf.convert_to_tensor([self.item_category[i] for i in range(self.num_items)],
                                                         dtype=tf.int32)
    ...
    def _create_optimizer(self):
        with tf.name_scope("optimizer"):
            ...
            opt4 = tf.compat.v1.train.AdagradOptimizer(lr[3])
            opt5 = tf.compat.v1.train.AdagradOptimizer(lr[4])
