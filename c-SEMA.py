...
def attack(data, attack_type):
    ...
    item_csv = pd.read_csv(rf"/home/lxh/wcy/TAaMR-master/data/{param.dataset}/original_images/classes.csv")
    for index, (user_id, user_info) in enumerate(data.items(), start=1):
        ...
        target_class = item_csv['ClassStr'][target]
        if ',' in target_class:
        ...
        for item in user_info['item_set']:
            ...
            attack_img = Image.open(os.path.join(param.file_path, file_lis[item]))
            target_img = Image.open(os.path.join(param.file_path, file_lis[target]))
            max_size = max(attack_img.width, attack_img.height)
            scaling_factor = 100 / max_size
            new_size = (int(attack_img.width * scaling_factor), int(attack_img.height * scaling_factor))
            attack_img_resized = attack_img.resize(new_size)
            canvas_height = max(target_img.height, attack_img_resized.height)

            text_measure_image = Image.new('RGB', (200, 200), (255, 255, 255))
            text_measure_draw = ImageDraw.Draw(text_measure_image)
            text = f"Match your \n{target_class}!"
            # Define the font (optional, you can choose a font file you have)
            try:
                font = ImageFont.truetype("arial.ttf", 10)
            except IOError:
                font = ImageFont.load_default()
            bbox = text_measure_draw.textbbox((0, 0), text, font=font)
            text_width, text_height = bbox[2] - bbox[0], bbox[3] - bbox[1]
            del text_measure_image
            del text_measure_draw
            if attack_img_resized.width > text_width * 1.2:
                canvas_width = int(target_img.width + attack_img_resized.width)
            else:
                canvas_width = int(target_img.width + text_width * 1.2)

            canvas = Image.new('RGB', (canvas_width, canvas_height), (255, 255, 255))
            canvas.paste(target_img, (0, 0))
            canvas.paste(attack_img_resized, (target_img.width, canvas_height - 40 - attack_img_resized.height))

            draw = ImageDraw.Draw(canvas)

            text_x = target_img.width + (canvas_width - target_img.width - text_width) * 0.5
            text_y = canvas.height - 40

            draw.text((text_x, text_y), text, fill=(0, 0, 0), font=font)
            img_file_path = f'./results/{param.model}/{param.dataset}/{attack_type}/Subs_{param.arch}-Rec_{param.rec_arch}/user_id{user_id}'
            save_img = os.path.join(img_file_path, f'composite_image_{item}.jpg')
            os.makedirs(img_file_path, exist_ok=True)
            canvas.save(save_img)

            if param.rec_arch == 'tensorflow_file':
                feature_adv_former = Feature_Extract(attack_img, rec_feature_model, tpdv,
                                                     param.rec_arch,
                ...
                score, adv_index, ori_index = ori_prediction(
                    Feature_Extract(attack_img, rec_feature_model, tpdv, tensor_feature=True).detach(),
            ...
            img_info['SSIM_Value'].append(f'{1.0}')
    ...
    print(f"WSS_Average:{WSS_average}")
