...
class Categorical(nn.Module):
    def __init__(self, num_inputs, num_outputs, gain=0.01):
        ...
        def init_(m):
            return init(m, nn.init.orthogonal_, lambda x: nn.init.constant_(x, 0), gain)  # nn.init.orthogonal_这里指的是用正交矩阵作为神经网络的初始化权重，将偏置设置为0，lambda x: nn.init.constant_(x, 0)，gain表示的是缩放因子，用来控制初始化权重的比例
